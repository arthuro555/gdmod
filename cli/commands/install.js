...
module.exports = (cli) => {
  class Install extends Command {
    ...
    static usage = {
      ...
      examples: [[`Launch the interactive installer`, `$0 install`]],
    };
    ...
    async execute() {
      ...
      const results = await require("inquirer").prompt([
        {
          type: "list",
          name: "type",
      ...
      if (results.type === "web") {
        console.log(
          chalk.green`
The CLI cannot patch games hosted online. Please use the browser extension for this platform.
` +
            chalk.bold`Read more at ` +
            chalk.underline.italic
              .grey`https://github.com/arthuro555/gdmod/wiki/Installation-Guide#web-games`
        );
        return 1;
      }

      const command = ["install", results.type, results.location];
  ...
  class InstallLoaderDir extends Command {
    ...
    static usage = {
      ...
      details: "",
      examples: [[`Patch an exported GDevelop game`, `$0 install dir`]],
    };
  ...
  class InstallLoaderAsar extends Command {
    ...
    static usage = {
      ...
      details: "",
      examples: [[`Patch an asar file`, `$0 install asar ./app.asar`]],
    };
